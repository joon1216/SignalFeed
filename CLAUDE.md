...
│   │   ├── card_renderer.py     # CardScript → HTML 5장 (커버 + 내지, 아이보리 디자인)
...
│   ├── reference/
│   │   └── collect.py           # 벤치마크 콘텐츠 수집 (instaloader/yt-dlp, 키 불필요)
...
5. **디자인** — 미니멀 아이보리/크리미 베이지(#F8F6F0 계열, 순백 금지), 폰 기준 큰 텍스트,
   커버는 다크+어그로 훅(순한국어). 1080x1350 (4:5), 5장.
